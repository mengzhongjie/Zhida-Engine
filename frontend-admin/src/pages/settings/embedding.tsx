...
import { Alert, Button, Card, Form, Input, InputNumber, Modal, Popconfirm, Radio, Select, Space, Tag, Typography, message } from 'antd'
...
type Profile = { id: number; name: string; provider_id: string; provider_name: string; mode: 'local'|'cloud'; local_model: string; local_device: string; cloud_base_url: string; cloud_api_key: string; cloud_model: string; cloud_dimension: number; model: string; dimension?: number; is_primary: boolean; is_active: boolean; last_test_success: boolean|null; last_error: string|null }
...
export default function EmbeddingSettings() {
  ...
  useEffect(() => { load() }, [])
  const edit = (item?: Profile) => { setEditing(item || null); const provider = item && providers.find(p => p.provider_id === item.provider_id); setModels(provider?.available_models || []); form.setFieldsValue(item ? { ...item, cloud_api_key: '' } : { name: '向量模型', mode: 'local', local_model: 'BAAI/bge-large-zh-v1.5', local_device: 'cpu', cloud_dimension: 1024 }); setOpen(true) }
  ...
  return <div><div className="page-header"><div><Button type="text" icon={<ArrowLeftOutlined/>} onClick={()=>navigate('/settings')} style={{marginLeft:-8}}>返回设置</Button><Title level={3}>向量化模型</Title><Text type="secondary">可以保存多套配置，运行时使用当前主模型。</Text></div><Space><Button icon={<ReloadOutlined/>} loading={rebuilding} onClick={confirmRebuild}>重建索引</Button><Button type="primary" icon={<PlusOutlined/>} onClick={()=>edit()}>新增配置</Button></Space></div>
    ...
    <Modal title={editing?'编辑向量配置':'新增向量配置'} open={open} onCancel={()=>setOpen(false)} footer={null} destroyOnHidden><Form form={form} layout="vertical"><Form.Item name="name" label="配置名称" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="mode" label="模式" rules={[{required:true}]}><Radio.Group><Radio value="local">本地模型</Radio><Radio value="cloud">云端 API</Radio></Radio.Group></Form.Item><Form.Item noStyle shouldUpdate={(a,b)=>a.mode!==b.mode}>{({getFieldValue})=>getFieldValue('mode')==='local'?<><Form.Item name="local_model" label="本地模型" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="local_device" label="设备"><Select options={[{value:'cpu',label:'CPU'},{value:'cuda',label:'CUDA'}]}/></Form.Item></>:<><Form.Item name="provider_id" label="厂商" rules={[{required:true}]}><Select onChange={providerChanged} options={providers.map(p=>({value:p.provider_id,label:p.name}))}/></Form.Item><Form.Item name="provider_name" hidden><Input/></Form.Item><Form.Item name="cloud_base_url" label="API 地址" rules={[{required:true}]}><Input/></Form.Item><Form.Item name="cloud_api_key" label="API Key" extra={editing?.cloud_api_key?`当前：${editing.cloud_api_key}；留空表示不修改`:''}><Input.Password autoComplete="new-password"/></Form.Item><Form.Item name="cloud_model" label="模型" rules={[{required:true}]}>{models.length?<Select onChange={modelChanged} options={models.map(m=>({value:m.model,label:`${m.model} (${m.dimension}维)`}))}/>:<Input/>}</Form.Item><Form.Item name="cloud_dimension" label="向量维度" rules={[{required:true}]}><InputNumber min={1} style={{width:'100%'}}/></Form.Item></>}</Form.Item><Button type="primary" onClick={save} loading={saving}>保存配置</Button></Form></Modal>
  </div>
}
